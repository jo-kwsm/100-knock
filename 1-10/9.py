...
def main():
    ...
    print(data["transaction_detail"].groupby(["payment_month", "item_name"]).sum()[["price", "quantity"]])

    pivot_table = pd.pivot_table(data["transaction_detail"], index="item_name", columns="payment_month", values=["price", "quantity"], aggfunc="sum")
    print(pivot_table)
